...
from app.core.user import CurrentUser, AdminOrOwner
...
async def delete_api_key(key_id: int, user: AdminOrOwner, session: db_session, redis: redis_client):

    return await revoke_user_api_key(session, user.user_id, key_id, redis)


@router.get("")
async def get_my_keys(user: AdminOrOwner, session: db_session):
